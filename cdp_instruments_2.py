...
def get_all_parametrs(p, t, p_bbp, t_reservoir, gas_saturation, y_a, y_c1, y_c, density_oil_without_gas, relative_density_gas, density_water, B, q_liquid, D_tubing, a_gas = 0):
...
def get_crd(q_liquid, 
    ...
    for p, t in zip(p_list, t_list):
        ...
        if len(list_all_parametrs) == 0:
            all_parametrs = get_all_parametrs(p, t, p_bbp, t_reservoir, gas_saturation, y_a, y_c1, y_c, density_oil_without_gas, relative_density_gas, density_water, B, q_liquid, D_tubing)
...
def get_crd_from_head(q_liquid,
    ...
    for p, t in zip(p_list, t_list):
        ...
        if h<h_tubing:
            ...
            D_tubing = D_casing_in
        if len(list_all_parametrs) == 0:
            all_parametrs = get_all_parametrs(p, t, p_bbp, t_reservoir, gas_saturation, y_a, y_c1, y_c, density_oil_without_gas, relative_density_gas, density_water, B, q_liquid, D_tubing, a_gas)
            list_all_parametrs.append(all_parametrs)
